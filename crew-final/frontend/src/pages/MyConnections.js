...
export default function MyConnections() {
  ...
  // useCallback so the function reference is stable for useEffect dependency
  const fetchAll = useCallback(async () => {
    ...
    setMatches((mRes.data || []).map(r => r.user1_id === user.id ? { ...r, partner: r.user2 } : { ...r, partner: r.user1 }));
  ...
  useEffect(() => {
    ...
    const connectSub = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'connect_requests' }, () => {
        fetchAll();
      })
      .subscribe();
  ...
  const handleAccept = async (req) => {
    const { error: updateErr } = await supabase.from('connect_requests').update({ status: 'accepted' }).eq('id', req.id);
    if (updateErr) { toast.error('Could not accept request.'); return; }
    const { error: matchErr } = await supabase.from('matches').insert({ user1_id: req.from_user_id, user2_id: req.to_user_id });
  ...
  const handleUnsave = async (savedUserId) => {
    await supabase.from('saved_profiles').delete().eq('user_id', user.id).eq('saved_user_id', savedUserId);
  ...
  const ProfileCard = ({ p, actions }) => (
    <div className="rounded-[16px] p-4 border flex items-center gap-3 transition-all hover:-translate-y-0.5"
      ...
      {p?.photo_url ? <img src={p.photo_url} alt={p.name} className="w-11 h-11 rounded-full object-cover" /> : <GradientAvatar name={p?.name} />}
      <div className="flex-1 min-w-0">
        <Link to={`/athlete/${p?.id}`} className="font-inter font-semibold text-sm text-white hover:text-amber-brand truncate block">{p?.name || 'Unknown'}</Link>
            ...
            <div className="space-y-3">
              {tab === 'matches' && (matches.length > 0 ? matches.map(m => (
                <ProfileCard key={m.id} p={m.partner} actions={
                  <a href={`https://wa.me/${(m.partner?.phone || '').replace(/\D/g, '')}?text=Hey!+We+matched+on+CREW+%E2%80%94+want+to+train%3F`}
                    ...
                    style={{ background: '#25D366', color: '#fff' }} data-testid={`whatsapp-${m.partner?.id}`}>
              ...
              {tab === 'received' && (recvReqs.length > 0 ? recvReqs.map(r => (
                <ProfileCard key={r.id} p={r.from_profile} actions={<>
                  <button onClick={() => handleAccept(r)} className="px-3 py-1.5 rounded-pill font-inter font-semibold text-xs"
                    style={{ background: '#D4880A', color: '#fff' }} data-testid={`accept-${r.id}`}>
                    <Check size={14} />
                  </button>
                  <button onClick={() => handleDecline(r)} className="px-3 py-1.5 rounded-pill font-inter text-xs"
                    style={{ border: '2px solid rgba(74,61,143,0.30)', color: 'rgba(255,255,255,0.5)' }} data-testid={`decline-${r.id}`}>
                    <X size={14} />
                  </button>
                </>} />
              ...
              {tab === 'sent' && (sentReqs.length > 0 ? sentReqs.map(r => (
                <ProfileCard key={r.id} p={r.to_profile} actions={
                  <button onClick={() => handleWithdraw(r)} className="font-inter text-xs hover:text-amber-brand flex items-center gap-1"
                    style={{ color: 'rgba(255,255,255,0.4)' }} data-testid={`withdraw-${r.id}`}>
              ...
              {tab === 'saved' && (saved.length > 0 ? saved.map(s => (
                <ProfileCard key={s.id} p={s.saved} actions={
                  <div className="flex gap-2">
                    <Link to={`/athlete/${s.saved?.id}`} className="px-3 py-1.5 rounded-pill font-inter font-semibold text-xs"
                      style={{ background: '#D4880A', color: '#fff' }}>View</Link>
                    <button onClick={() => handleUnsave(s.saved?.id)} className="px-3 py-1.5 rounded-pill font-inter text-xs"
