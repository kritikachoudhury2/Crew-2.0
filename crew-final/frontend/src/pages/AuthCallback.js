...
export default function AuthCallback() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (!resolved) { setError('Sign-in timed out. Please try again.'); subscription.unsubscribe(); }
    }, 12000);

    return () => { clearTimeout(fallbackTimer); clearTimeout(timeout); subscription.unsubscribe(); };
  ...
  if (error) {
    return (
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100vh', background:'#1C0A30', gap:16, padding:24 }}>
        ...
        <p style={{ color:'#ef4444', fontFamily:'Inter, sans-serif', fontSize:15, fontWeight:600, textAlign:'center' }}>{error}</p>
        <p style={{ color:'rgba(255,255,255,0.5)', fontFamily:'Inter, sans-serif', fontSize:13, textAlign:'center' }}>Magic links expire after 1 hour. Request a new one to sign in.</p>
